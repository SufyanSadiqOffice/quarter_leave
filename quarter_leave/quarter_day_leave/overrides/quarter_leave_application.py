...
class QuarterLeaveApplication(LeaveApplication):
	def get_feed(self):
	...
	def validate_balance_leaves(self):
		if self.from_date and self.to_date:
			self.total_leave_days = get_number_of_leave_day(
				self.employee,
				self.leave_type,
				self.from_date,
				self.to_date,
	...
	def get_consecutive_leave_details(self) -> dict:
		...
		total_consecutive_leaves = get_number_of_leave_day(
	...
	def create_separate_ledger_entries(self, alloc_on_from_date, alloc_on_to_date, submit, lwp):
		...
		leaves_in_first_alloc = get_number_of_leave_day(
			self.employee,
			self.leave_type,
			self.from_date,
			first_alloc_end,
		...
		leaves_in_second_alloc = get_number_of_leave_day(
			self.employee,
			self.leave_type,
			second_alloc_start,
			self.to_date,
...
def get_approved_leaves_for_period(employee, leave_type, from_date, to_date):
	...
	for leave_app in leave_applications:
		if leave_app.from_date >= getdate(from_date) and leave_app.to_date <= getdate(to_date):
			...
			leave_days += get_number_of_leave_day(
